...
```{r angle-regression-sim, fig.width = 6, fig.height = 2, cache = FALSE, fig.cap = 'The average MSE over 50 simulations in scenario 1. The errorbars are the standard error.'}
n.sigma <- 1
...
```{r theta-mse, fig.width = 6, fig.height = 2, cache = FALSE, fig.cap = 'The average MSE of the estimated angles in scenario 1 over 50 simulations. The errorbars are the standard error.'}
angle.reg.df %>% 
  dplyr::filter(n >= 128) %>% 
  dplyr::group_by(n) %>% 
...
```{r beta-regression-sim, fig.width = 6, fig.height = 2, cache = FALSE, fig.cap = 'The average MSE over 50 simulations in scenario 2. The errorbars are the standard error.'}
...
```{r beta-mse, fig.width = 6, fig.height = 2, cache = FALSE, fig.cap = 'The average MSE of the estimated beta parameters, $(a, b)$, in scenario 2 over 50 simulations. The errorbars are the standard error.'}
beta.reg.df %>% 
  dplyr::group_by(n) %>% 
...
In this section, we apply \ref{alg:mslm-fit} to real data to see if we can extract estimates from the latent space that correspond to response variables. 
