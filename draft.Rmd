...
There exists much existing literature on the estimation for multilayer networks, such as the multilayer degree corrected block model of \citet{agterberg2022joint}, multiple random eigen graphs of \citet{8889404}, and the common subspace independent edge graphs of \citet{arroyo2020inference}. 
...
To use the MLSM for regression problems, we assign response variables $y_1, ..., y_L$ to each graph. 
...
We also vary the expected number of vertices for each graph: For each $N = 128, 256, 512$, and $1024$, we sample $L = 128$ graphs as $n_1, ..., n_L \iid \Poisson(N)$. 
Algorithm \ref{alg:mlsm-fit} is applied to each set of $128$ graphs, then half of the estimates are used for fitting the linear regression model and the mean squared error is computed on the remainder. 
To compare our algorithm against existing methods, we extracted graph statistics as covariates for a linear regression model. 
For each $N$, we ran $50$ replicates to obtain the average and standard error of each method. 

```{r angle-regression-sim, fig.width = 5, fig.height = 2, cache = FALSE, fig.cap = 'The average MSE over 50 simulations in scenario 1. The errorbars are the standard error.'}
n.sigma <- 1
...
angle.reg.df %>% 
  ...
  geom_line(aes(x = n, y = mse.theta, colour = 'Estimated \nangle \n')) + 
  ...
  #                   colour = 'modularity'), 
  #               width = .1) + 
  geom_line(aes(x = n, y = mse.a, colour = 'Graph \nstatistics')) + 
  geom_errorbar(aes(x = n,
                    ymin = mse.a - se.a,
                    ymax = mse.a + se.a,
                    colour = 'Graph \nstatistics'), 
...
```{r theta-mse, fig.width = 4, fig.height = 2, cache = FALSE, fig.cap = 'The average MSE of the estimated angles in scenario 1 over 50 simulations. The errorbars are the standard error.'}
angle.reg.df %>% 
  dplyr::filter(n >= 128) %>% 
  dplyr::group_by(n) %>% 
...
```{r beta-regression-sim, fig.width = 5, fig.height = 2, cache = FALSE, fig.cap = 'The average MSE over 50 simulations in scenario 2. The errorbars are the standard error.'}
...
beta.reg.df %>% 
  ...
  dplyr::summarise(mse.beta = mean(mse),
                   ...
                   mse.t = mean(mse.t)) %>% 
  ggplot() + 
  geom_line(aes(x = n, y = mse.beta, colour = 'Estimated \nbeta \nparameters\n')) + 
  geom_errorbar(aes(x = n, 
                    ymin = mse.beta - se.beta, 
                    ymax = mse.beta + se.beta,
                    colour = 'Estimated \nbeta \nparameters\n'),
                width = .1) + 
  geom_line(aes(x = n, y = mse.d, colour = 'Graph \nstatistics')) + 
  geom_errorbar(aes(x = n,
                    ymin = mse.d - se.d,
                    ymax = mse.d + se.d,
                    colour = 'Graph \nstatistics'),
...
```{r beta-mse, fig.width = 5, fig.height = 2, cache = FALSE, fig.cap = 'The average MSE of the estimated beta parameters, $(a, b)$, in scenario 2 over 50 simulations. The errorbars are the standard error.'}
beta.reg.df %>% 
  dplyr::group_by(n) %>% 
  ...
  geom_errorbar(aes(x = n, ymin = b.mse - b.se, ymax = b.mse + b.se, colour = 'b'),
                width = .1) + 
  theme_bw() + 
  labs(x = 'number of vertices', y = 'MSE of Beta \nparameter estimates', 
       colour = 'Beta \ndistribution \nparameter') + 
  scale_colour_brewer(palette = 'Set1') +
  scale_y_log10() + 
  scale_x_log10(breaks = sort(n.vec))
...
In figure \ref{fig:angle-regression-sim}, we compare algorithm \ref{alg:mlsm-fit} to fitting a regression model using graph transitivity and average degree of each vertex (scenario 1). 
...
In figure \ref{fig:beta-regression-sim}, we see that linear regression using graph summary statistics performs better for smaller graphs, but as we draw more vertices, the ASE improves and so algorithm \ref{alg:mlsm-fit} performs better, whereas graph summary statistics cannot improve with the number of vertices. 
...
In this section, we apply algorithm \ref{alg:mslm-fit} to real data to see if we can extract estimates from the latent space that correspond to response variables. 
In the first example, we analyzed the larval *Drosophila* mushroom body connectome \citep{Eichler141762}, which has been studied as a GRDPG by \citet{athreya2020estimation}. 
...
```{r}
pairwise.angles.df <- readr::read_csv('data/pairwise-angles.csv')
