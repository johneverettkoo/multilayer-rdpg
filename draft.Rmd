...
The second setting involves a nonlinear latent structure that can be described as a polynomial curve and thus does not reduce to a simpler model or be analyzed by their corresponding methods. 

The scenarios are as follows:

1. In this setting, there are two latent structures, described by two rays emanating from the origin. 

2. 

## Regression

In this scenario, the responses $y_\ell$ are generated as linear responses of $\theta_\ell$ (in scenario 1) or $a_\ell$ and $b_\ell$ (in scenario 2). 
More specifically, in scenario 1, each $y_\ell \indep \mathcal{N}(\beta_0 + \beta_1 \theta_\ell, \sigma^2)$, and in scenario 2, each $y_\ell \indep \mathcal{N}(\beta_0 + \beta_1 a_\ell + \beta_2 b_\ell, \sigma^2)$.
...
angle.reg.df %>% 
  ...
  labs(x = 'vertices per graph', y = 'MSE', colour = 'predictor') +
...
angle.reg.df %>% 
  ...
  geom_errorbar(aes(x = n, ymin = theta.mse - theta.se, ymax = theta.mse + theta.se),
                width = .1) + 
  labs(x = 'number of vertices', y = expression('MSE('~hat(theta)~')')) + 
  theme_bw() + 
  scale_y_log10() + 
  scale_x_log10(breaks = sort(n.vec))
```
...
beta.reg.df %>% 
  ...
  scale_colour_brewer(palette = 'Set1') +
  scale_x_log10(breaks = sort(n.vec)) + 
  labs(x = 'number of vertices', y = 'MSE', colour = 'predictor')
```
